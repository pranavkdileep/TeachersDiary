...
if(isset($_POST['update'])) {
    ...
    if(empty($marked_at) || empty($subjectid) || empty($teacherid) || empty($hour) || empty($all_students)) {
        echo "<script>alert('Missing required fields.'); window.location='studentattendanceupdate.php';</script>";
        exit();
    }
    ...
    foreach($all_students as $universityid) {
        ...
        if($exists) {
            ...
            // Insert new record (if no previous record exists)
    ...
    if($success) {
        ...
        echo "<script>alert('Attendance updated successfully for Hour $hour.');
        window.location='studentattendanceupdate.php?txtdate=$marked_at&ddlsubject=$subjectid&ddlhour=$hour';</script>";
    } else {
        $obj->executequery("ROLLBACK");
        echo "<script>alert('Failed to update attendance for Hour $hour.');
        window.location='studentattendanceupdate.php?txtdate=$marked_at&ddlsubject=$subjectid&ddlhour=$hour';</script>";
    }
} else {
    echo "<script>alert('Invalid request.');
    window.location='studentattendanceupdate.php';</script>";
}
?>